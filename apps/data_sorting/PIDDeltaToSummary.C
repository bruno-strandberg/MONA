...
int main(const int argc, const char **argv) {
  ...
  try {
    ...
    JParser<> zap("This program takes the ECAP PID summary file as input, reading it with PIDDelta.h/C class, and converts it to `SummaryEvent` format for usage with NMH software.");

    zap['f'] = make_field(fin_name , "PID file from ECAP with the PID TTree that was used to create PIDDelta.h/C class") = "../../data/pid_result_6May2019_ORCA115_23x9m.root";
  ...
  if (fin_name == "" || fout_dir  == "" || tag == "") {
    throw std::invalid_argument("ERROR! PIDDeltaToSummary() all command line arguments need to be specified!");
  }
  ...
  if (tin == NULL) {
    throw std::invalid_argument("ERROR! PIDDeltaToSummary() cannot find tree PID in file " + fin_name);
  }
  PIDDelta PIDR(tin);
  ...
  cout << "NOTICE PIDDeltaToSummary() creating file " << fout_name << endl;
  ...
  for (Int_t i = 0; i < nentries; i++) {
    ...
    //********************************************************************************
     ...
     out.GetEvt()->Set_MC_runID(PIDR.run_from_header);       
     ...
     out.GetEvt()->Set_RDF_muon_score(0.); // Removed in the 190222 production, will be there later
     out.GetEvt()->Set_RDF_track_score(PIDR.track_score_Orca23m_190222_hitpdf_all);
     //out.GetEvt()->Set_RDF_noise_score(PIDR.noise_score);
     out.GetEvt()->Set_RDF_noise_score(0.); // Removed in the 190222 production, will be there later
  ...
  FileHeader head("PIDDeltaToSummary");
