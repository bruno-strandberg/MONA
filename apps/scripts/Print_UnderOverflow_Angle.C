...
void Print_UnderOverflow_Angle(TString summary_file=(TString)getenv("NMHDIR") + "/data/ORCA_MC_summary_all_10Apr2018.root") { 
  ...
  for (Int_t i = 0; i < sp.GetTree()->GetEntries(); i++) {
    ...
    if ((std::abs(evt->Get_MC_type()) == 11) or (std::abs(evt->Get_MC_type()) == 12)) { // Abs for anti-particles
      if (evt->Get_MC_is_CC()) h2elcc[index]->Fill(-evt->Get_MC_dir_z(), -evt->Get_shower_dir_z());
      else                     h2elnc[index]->Fill(-evt->Get_MC_dir_z(), -evt->Get_shower_dir_z());
    }
    if ((std::abs(evt->Get_MC_type()) == 13) or (std::abs(evt->Get_MC_type()) == 14)) {
      if (evt->Get_MC_is_CC()) h2mucc[index]->Fill(-evt->Get_MC_dir_z(), -evt->Get_track_dir_z());
      else                     h2munc[index]->Fill(-evt->Get_MC_dir_z(), -evt->Get_shower_dir_z());
    }
    if ((std::abs(evt->Get_MC_type()) == 15) or (std::abs(evt->Get_MC_type()) == 16)) {
      if (evt->Get_MC_is_CC()) h2tacc[index]->Fill(-evt->Get_MC_dir_z(), -evt->Get_shower_dir_z());
      else                     h2tanc[index]->Fill(-evt->Get_MC_dir_z(), -evt->Get_shower_dir_z());
    }
  }
  if (print) {
