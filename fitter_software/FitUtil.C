...
FitUtil::~FitUtil() {
  ...
  TAxis* eaxis  = fHB->GetXaxis();
  TAxis* ctaxis = fHB->GetYaxis();
...
void FitUtil::FillFluxCache(AtmFlux *flux, Double_t op_time, UInt_t nsamples) {
  ...
  for (UInt_t f = 0; f < fFlavs; f++) {
    for (UInt_t isnb = 0; isnb < fPols; isnb++) {
      ...
      TH3D* hflux = fHB; // get the true bin space template histogram
...
void FitUtil::ProbCacher(const proxymap_t &proxymap, UInt_t nsamples) {
  ...
  for (UInt_t f_in = 0; f_in < fFlavs-1; f_in++) {
	...
	Int_t N_ebins  = fHB->GetXaxis()->GetNbins();
	Int_t N_ctbins = fHB->GetYaxis()->GetNbins();
	...
	for (Int_t ebin = 1; ebin <= N_ebins; ebin++) {
	  for (Int_t ctbin = 1; ctbin <= N_ctbins; ctbin++) {
	    ...
	    Double_t emin    = fHB->GetXaxis()->GetBinLowEdge(ebin);
	    Double_t emax    = fHB->GetXaxis()->GetBinUpEdge(ebin);
	    Double_t e_step  = (emax - emin)/(nsamples+1);

	    Double_t ctmin   = fHB->GetYaxis()->GetBinLowEdge(ctbin);
	    Double_t ctmax   = fHB->GetYaxis()->GetBinUpEdge(ctbin);
