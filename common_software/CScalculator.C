...
CScalculator::CScalculator(TString xsecfile) {
  ...
  nu_types.insert( pair<Int_t, TString> {0, "e"}   );
  nu_types.insert( pair<Int_t, TString> {1, "mu"}  );
  nu_types.insert( pair<Int_t, TString> {2, "tau"} );
 ...
 *
 * \param  nu_type       Neutrino type
 * \param  is_cc         Interaction type (0 - nc, 1 - cc)
 * \param  is_nubar      0 - particle, 1 - antiparticle.
 *
 */
void CScalculator::SelectInteraction(Int_t nu_type, Bool_t is_cc, Bool_t is_nubar) {
  ...
  if ( nu_types.find(nu_type) == nu_types.end() ) {
    cout << "ERROR! CScalculator::SelectInteraction() neutrino type " << nu_type 
	 << " not supported" << endl;
    supported = false;
  }
  ...
  if (supported) {
    ...
    TString searchstr = CreateString(nu_type, is_cc, is_nubar);
...
Bool_t CScalculator::InitGraphs(TString xsecfile) {
  ...
  //for each combination of nu_type, int_type, p_type. Clone the xsec graphs to the map.
  
  for (auto& nutype: nu_types) {
    for (auto& inttype: int_types) {
      for (auto& ptype: p_types) {

	TString gname1 = "nu_" + nutype.second + ptype.second + "_H1/tot" + inttype.second;
	TString gname2 = "nu_" + nutype.second + ptype.second + "_O16/tot" + inttype.second;
	
	vector<TGraph*> graphs;
	graphs.push_back( (TGraph*)( f->Get(gname1)->Clone() ) );
	graphs.push_back( (TGraph*)( f->Get(gname2)->Clone() ) );
	
	TString namestr = CreateString( nutype.first, (Bool_t)inttype.first, (Bool_t)ptype.first );
 ...
 */
TString CScalculator::CreateString(Int_t nu_type, Bool_t is_cc, Bool_t is_nubar) {

  return nu_types[nu_type] + p_types[(Int_t)is_nubar] + int_types[(Int_t)is_cc];
