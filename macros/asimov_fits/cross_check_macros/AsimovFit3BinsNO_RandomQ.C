...
void AsimovFit3BinsNO_RandomQ() {
  ...
  TString s_outputfile = TString::Format("output/csv/CrossCheck/random_q/AsimovFit3BinsNO_RandomQ_Dist_Rand3bins.csv");
  ...
  outputfile << "n_chi2sh0_no,n_chi2sh1_no,n_chi2_tr,fit_chi2" << endl;
  ...
  outputfile << std::get<1>(chi2[0]) << "," << std::get<1>(chi2[1]) << "," << std::get<1>(chi2[2]) << "," << min_chi2 << endl;
  outputfile.close();
