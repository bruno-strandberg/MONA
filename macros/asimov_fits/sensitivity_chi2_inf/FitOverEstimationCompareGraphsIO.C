...
void FitOverEstimationCompareGraphsIO() {
  ...
  for (auto p: plotInf) {
    ...
    if (plot_inf) {
      ...
      legend_label = "IO, %i PID categories";
    ...
    TTree* t_2  = (TTree*)f->Get(s_tree + "2bins_no");
    TTree* t_3  = (TTree*)f->Get(s_tree + "3bins_no");
    TTree* t_4  = (TTree*)f->Get(s_tree + "4bins_no");
    TTree* t_5  = (TTree*)f->Get(s_tree + "5bins_no");
    TTree* t_10 = (TTree*)f->Get(s_tree + "10bins_no");
    TTree* t_21 = (TTree*)f->Get(s_tree + "21bins_no");
    TTree* t_22 = (TTree*)f->Get(s_tree + "22bins_no");
    TTree* t_31 = (TTree*)f->Get(s_tree + "31bins_no");
    TTree* t_32 = (TTree*)f->Get(s_tree + "32bins_no");
    TTree* t_41 = (TTree*)f->Get(s_tree + "41bins_no");
  ...
  //    g10->SetMarkerStyle(21);
    ...
    fout->cd();
    g2->Write("comaprison_2bins_io_" + out_name);
    g3->Write("comaprison_3bins_io_" + out_name);
    g4->Write("comaprison_4bins_io_" + out_name);
    g5->Write("comaprison_5bins_io_" + out_name);
    g10->Write("comaprison_10bins_io_" + out_name);
    g21->Write("comaprison_21bins_io_" + out_name);
    g22->Write("comaprison_22bins_io_" + out_name);
    g31->Write("comaprison_31bins_io_" + out_name);
    g32->Write("comaprison_32bins_io_" + out_name);
    g41->Write("comaprison_41bins_io_" + out_name);
    fout->Close();

    if (plot_inf) c1->SaveAs("CompareChi2AtInfStatisticsIO.pdf");
