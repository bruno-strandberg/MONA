...
/* Macro to generate the detector responses in the 3 PID scheme: there are 3 pid classes
 * defined at the start and they are showers, bad showers and tracks. For all classes under 0.6
...
void DetectorResponseSplit3Bins() {
  ...
  TString filefolder = TString::Format("./pid_detres/pid_binning_%i/qcut_0.4/", N_PID_CLASSES);
  ...
  for (int i = 0; i < N_PID_CLASSES; i++) {
    ...
    DetResponse track_response(DetResponse::track, TString::Format("track_response_%.2f", pid_map[i]), 40, 1, 100, 40, -1, 1, 1, 0, 1);
    ...
    DetResponse shower_response(DetResponse::shower, TString::Format("shower_response_%.2f", pid_map[i]), 40, 1, 100, 40, -1, 1, 1, 0, 1);
    ...
    DetResponse mc_response(DetResponse::mc_truth, TString::Format("mc_response_%.2f", pid_map[i]), 40, 1, 100, 40, -1, 1, 1, 0, 1);
    mc_response.AddCut( &SummaryEvent::Get_RDF_track_score, comparison_operator      , pid_map[i]  , true );
    mc_response.AddCut( &SummaryEvent::Get_RDF_track_score, std::less_equal<double>(), pid_map[i+1], true );
  ...
  for (int i = 0; i < N_PID_CLASSES; i++) {
    track_response_vector[i].WriteToFile(filefolder + TString::Format("track_response_%.2f.root" , pid_map[i]));
    shower_response_vector[i].WriteToFile(filefolder + TString::Format("shower_response_%.2f.root", pid_map[i]));
    mc_response_vector[i].WriteToFile(filefolder + TString::Format("mc_response_%.2f.root", pid_map[i]));
  }
  ...
  for (int i = 0; i < N_PID_CLASSES; i++){
    ...
    // save output
    //----------------------------------------------------------
    TString output_NO = TString::Format("split_expected_evts_NO_%.2f.root", pid_map[i]);
    ...
    TString output_IO = TString::Format("split_expected_evts_IO_%.2f.root", pid_map[i]);
