...
ORCA7::ORCA7(Bool_t ReadResponses) {
  ...
  // set the PID bins
  //-------------------------------------------------------------------------------
  fPidBins.push_back( PidBinConf(0.0, 0.6, 0.05, 0.01, "shw") );
  //fPidBins.push_back( PidBinConf(0.3, 0.7, 0.01, 0.0, "mid") );
  fPidBins.push_back( PidBinConf(0.6, 1.0+1e-5, 0.05, 0.01, "trk") );
  ...
  for (auto PB: fPidBins) {
    ...
    if ( PB.pid_min < 0.7 ) {
      R = new EvtResponse(EvtResponse::shower, PB.name+"R", f_T_ebins, f_T_emin, f_T_emax, f_T_ctbins, f_T_ctmin, f_T_ctmax, f_T_bybins, f_T_bymin, f_T_bymax, f_R_ebins, f_R_emin, f_R_emax, 
      ...
      R = new EvtResponse(EvtResponse::track, PB.name+"R", f_T_ebins, f_T_emin, f_T_emax, f_T_ctbins, f_T_ctmin, f_T_ctmax, f_T_bybins, f_T_bymin, f_T_bymax, f_R_ebins, f_R_emin, f_R_emax, 
			  f_R_ctbins, f_R_ctmin, f_R_ctmax, f_R_bybins, f_R_bymin, f_R_bymax);
  ...
  vector< std::pair<TString, EvtResponse*> > resp_names;// vector that stores the outname and the response pairs
...
*/
  ...
  //-------------------------------------------------------------------------------
  fFitUtil = new FitUtil(f_F_runtime, fResps["trk"]->GetHist3DTrue(), fResps["trk"]->GetHist3DReco(), f_F_emin, f_F_emax, 
			      f_F_ctmin, f_F_ctmax, f_F_bymin, f_F_bymax, "dummy");
