...
void SplitPID() {
  ...
  for (int i = 0; i < N_PID_CLASSES; i++) {
    ...
    DetResponse drt(DetResponse::track, TString::Format("track_response_%.2f", PID_step * i), 40, 1, 100, 40, -1, 1, 1, 0, 1);
    ...
    drt.AddCut( &SummaryEvent::Get_RDF_track_score , comparison_operator      , PID_step * i    , true );
    ...
    DetResponse drs(DetResponse::shower, TString::Format("shower_response_%.2f", PID_step * i), 40, 1, 100, 40, -1, 1, 1, 0, 1);
    ...
    DetResponse drm(DetResponse::mc_truth, TString::Format("mc_response_%.2f", PID_step * i), 40, 1, 100, 40, -1, 1, 1, 0, 1);
    ...
    drm.AddCut( &SummaryEvent::Get_RDF_noise_score, std::less_equal<double>(), 0.5             , true ); // Do these cuts make sense?? Yes, we want to compare the DR with "real DR"
    drms.push_back(drm);
  }
  ...
  for (int i = 0; i < N_PID_CLASSES; i++) { 
    if (writeFiles) { // This flag is to not accidently overwrite with empty files
      drts[i].WriteToFile(filefolder + TString::Format("track_response_%.2f.root" , PID_step * i));
...
//  for (int i = 0; i < N_PID_CLASSES; i++) {
//    drts[i].ReadFromFile(filefolder + TString::Format("track_response_%.2f.root" , PID_step * i));
...
//  }  
  ...
  for (int i = 0; i < N_PID_CLASSES; i++){
    ...
    for (auto order: orderings) {
      ...
      TString output = TString::Format("split_pid_%s_%.2f.root", order_string.c_str(), PID_step * i);
