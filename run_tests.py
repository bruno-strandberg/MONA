...
TESTS_DIR = ['./tests/common_software', './tests/fitter_software/']
JUNIT_XML = 'junit.xml'
...
def main():
    ...
    print("\n{}"
          ...
          "Failed tests: {}{}\n"
          "Elapsed time: {:.1f}s\n"
          .format(INFO, RST, n_tests,
                  BOLD+FAIL if n_failed_tests > 0 else OK,
...
def write_junit_xml(test_results):
    ...
    for test_script, (exit_code, t, stdout, stderr) in test_results.items():
        test_case = TestCase(test_script, elapsed_sec=t,
...
def print_captured_output(test_results):
    ...
    for test_script, (exit_code, t, stdout, stderr) in test_results.items():
        if exit_code > 0:
            ...
            print('{}stdout:{}\n{}\n{}stderr:{}\n{}'
                  .format(OK+BOLD, RST, stdout.decode('utf-8'),
                          FAIL+BOLD, RST, stderr.decode('utf-8')))
